...
#====================== STEP 6 UDER INPUT==============
st.header("write the Prompt to Generate PPT or Image oe Fetch Latest News")
...
def agent_prompt(query):
  ...
  query, support user needs PPT based on given query by user, it give detailed Professional prompt 
  ...
  professional HTML output prompt with no markdowns
  User Query: {query}"""
...
def run_agent(leader_agent, query):
  ...
  and imbed in same html ppt and using file handling embed this in 
  output html, use java script function to generate image using async
  and threading and give output in HTML User Query given below:"""
  prompt += query
